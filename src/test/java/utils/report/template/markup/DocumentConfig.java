...
import utils.report.template.Chart;
import utils.report.template.LogSettings;
...
public class DocumentConfig extends LogSettings {
	...
	/**
	 * Choose to use your own icons for log status (PASS, FAIL, WARNING etc.) by
	 * choosing one of the icons from Fontawesome website:
	 * http://fortawesome.github.io/Font-Awesome/icons/.
	 * 
	 * @param status
	 *            LogStatus
	 * @param newIcon
	 *            Icon string from font-awesome
	 ...
	 * 
	 * @param display
	 *            Setting FALSE will remove the TH headers from the test table
	 * @return DocumentConfig
	 */
	@Deprecated
	...
	/**
	 * Setting to show or hide the default Extent footer By default, this is
	 * FALSE
	 * 
	 * @param display
	 *            Setting FALSE will hide the default Extent footer
	 * @return DocumentConfig
	 */
	 ...
	 * 
	 * @param script
	 *            JavaScript code to be used for the report
	 * @return DocumentConfig
	 */
	 ...
	 * 
	 * @param cssFilePath
	 *            Path of the CSS stylesheet file
	 * @return DocumentConfig
	 */
	...
	public DocumentConfig documentTitle(String title) {
		...
		docHead.setFile(filePath).documentTitle(title);
	...
	public DocumentConfig renewSystemInfo() {
		...
		header.setFile(filePath).renewSystemInfo();

		return this;
	}

	/**
	 * Setting to append the caller class to the log event. By default, the
	 * setting is enabled to allow the caller class to be displayed
	 * 
	 * @param display
	 *            Shows the caller class in the report step
	 * @return DocumentConfig
	 */
	 ...
	 * 
	 * <p>
	 * <b>Usage: </b> extent.setImageSize("40%");
	 * 
	 * @param percent
	 *            Size of the image in percentage
	 * @return DocumentConfig
	 */
	 ...
	 *            TEST_SET, TEST
	 * @param title
	 *            New title of the chart
	 * @return DocumentConfig
	 */
	public DocumentConfig chartTitle(Chart chart, String title) {
		String html = FileReaderEx.readAllText(filePath);
		String currentTitle = ExtentMarkup.getPlaceHolder(chart.toString() + "chartTitle", "js", true) + ".*"
				+ ExtentMarkup.getPlaceHolder(chart.toString() + "chartTitle", "js", true);
		String match = RegexMatcher.getNthMatch(html, currentTitle, 0);

		html = html.replace(match, ExtentMarkup.getPlaceHolder(chart.toString() + "chartTitle", "js", false) + "'"
				+ title + "'" + ExtentMarkup.getPlaceHolder(chart.toString() + "chartTitle", "js", false));
	...
	/**
	 * @param filePath
	 *            Path of the HTML file with .htm or .html extension
	 */
