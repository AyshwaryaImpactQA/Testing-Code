...
import utils.report.template.support.*;
...
class Header {
	...
	public void setHeadline(String summary) {
		...
		String pattern = ExtentMarkup.getPlaceHolder("reportsummary") + ".*"
				+ ExtentMarkup.getPlaceHolder("reportsummary");
	...
	public void renewSystemInfo() {
		...
		String javaVersion = ExtentMarkup.getPlaceHolder("javaversion") + ".*"
				+ ExtentMarkup.getPlaceHolder("javaversion");
		...
		html = html.replace(temp,
				startedAt.replace(".*", new SimpleDateFormat("MM/dd HH:mm:ss").format(new Date()).toString()));
