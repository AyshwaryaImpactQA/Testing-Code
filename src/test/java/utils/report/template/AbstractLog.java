...
import utils.report.template.markup.ExtentMarkup;
...
abstract class AbstractLog {
	...
	public void log(LogStatus logStatus, String stepName, String details, String screenCapturePath) {
		...
		if (caller != null) {
			if (stepName != null)
				this.stepName = ExtentMarkup.createSingleNode("span", "caller-class", "[" + caller + "] ") + stepName;
			else
				this.details = ExtentMarkup.createSingleNode("span", "caller-class", "[" + caller + "] ") + details;
		}
	...
	public void startTest(String name, String description) {
		if (testCounter != 0)
			if (getLastRunStatus() == LogStatus.PASS)
				testsPassed++;
			else if (getLastRunStatus() == LogStatus.FAIL || getLastRunStatus() == LogStatus.FATAL
					|| getLastRunStatus() == LogStatus.ERROR)
				testsFailed++;
